...
addTaskButton.addEventListener("keydown", (vari) => {
  vari.preventDefault();
  if (vari.key === "Enter") {
...
function displayTodo() {
  ...
  todoArray.forEach((list, ind) => {
    htmlCode += `<div class='flex mb-4 items-center' id='task_box'>
...
saveTaskButton.addEventListener("click", (e) => {
  ...
  todoArray = JSON.parse(todo);
  console.log(todoArray);
  todoArray.splice(saveInd.value, 1, text.value);
  console.log(todoArray);
